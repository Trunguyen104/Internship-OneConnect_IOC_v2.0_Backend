...
{
    public class UpdateStakeholderIssueStatusHandlerTests
    {
        private readonly Mock<IUnitOfWork> _mockUnitOfWork;
        private readonly Mock<IMapper> _mockMapper;
        private readonly Mock<IMessageService> _mockMessageService;
        private readonly Mock<ILogger<UpdateStakeholderIssueStatusCommandHandler>> _mockLogger;
        private readonly Mock<ICurrentUserService> _mockCurrentUserService;
        private readonly Mock<ICacheService> _mockCacheService;
        private readonly UpdateStakeholderIssueStatusCommandHandler _handler;
        ...
        public UpdateStakeholderIssueStatusHandlerTests()
        {
            _mockUnitOfWork = new Mock<IUnitOfWork>();
            _mockMapper = new Mock<IMapper>();
            _mockMessageService = new Mock<IMessageService>();
            _mockLogger = new Mock<ILogger<UpdateStakeholderIssueStatusCommandHandler>>();
            ...
            // Default: UpdateAsync returns Task.CompletedTask
            _mockUnitOfWork.Setup(x => x.Repository<StakeholderIssue>().UpdateAsync(It.IsAny<StakeholderIssue>(), It.IsAny<CancellationToken>()))
                .Returns(Task.CompletedTask);

            // Default: mapper returns response
            _mockMapper.Setup(x => x.Map<UpdateStakeholderIssueStatusResponse>(It.IsAny<StakeholderIssue>()))
                .Returns(new UpdateStakeholderIssueStatusResponse { Id = _issueId, Status = StakeholderIssueStatus.InProgress });

            _handler = new UpdateStakeholderIssueStatusCommandHandler(
        ...
        public async Task Handle_IssueNotFound_ReturnsNotFound()
        {
            ...
            {
                Id = Guid.NewGuid(),
                Status = StakeholderIssueStatus.InProgress
            };
        ...
        [Fact]
        public async Task Handle_ValidStatus_ReturnsSuccess()
        {
            // Arrange
            var command = new UpdateStakeholderIssueStatusCommand
            {
            ...
            _mockUnitOfWork.Verify(x => x.Repository<StakeholderIssue>().UpdateAsync(It.IsAny<StakeholderIssue>(), It.IsAny<CancellationToken>()), Times.Once);
