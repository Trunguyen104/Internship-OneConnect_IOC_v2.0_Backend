...
namespace IOCv2.Application.Features.Jobs.Commands.ApplyJob
{
    public class ApplyJobHandler : MediatR.IRequestHandler<ApplyJobCommand, Result<ApplyJobResponse>>
    {
        ...
        public async Task<Result<ApplyJobResponse>> Handle(ApplyJobCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return Result<ApplyJobResponse>.Failure(_messageService.GetMessage(MessageKeys.JobPostingMessageKey.CvRequired), ResultErrorType.BadRequest);
            }

            // Placed check
            if (student.InternshipStatus == StudentStatus.INTERNSHIP_IN_PROGRESS || student.InternshipStatus == StudentStatus.COMPLETED)
            {
                return Result<ApplyJobResponse>.Failure(_messageService.GetMessage(MessageKeys.JobPostingMessageKey.AlreadyPlaced), ResultErrorType.Forbidden);
            }
            ...
            var hasActiveApp = student.InternshipApplications.Any(a =>
            ...
            var pastApplicationsForJobInTerm = student.InternshipApplications
            ...
            await _unitOfWork.Repository<InternshipApplication>().AddAsync(application, cancellationToken);
