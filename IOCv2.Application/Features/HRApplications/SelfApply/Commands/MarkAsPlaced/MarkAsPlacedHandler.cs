...
public class MarkAsPlacedHandler : IRequestHandler<MarkAsPlacedCommand, Result<MarkAsPlacedResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUserService;
    private readonly IMessageService _messageService;
    private readonly IPublisher _publisher;
    private readonly ILogger<MarkAsPlacedHandler> _logger;

    public MarkAsPlacedHandler(
        IUnitOfWork unitOfWork,
        ICurrentUserService currentUserService,
        IMessageService messageService,
        IPublisher publisher,
        ILogger<MarkAsPlacedHandler> logger)
    {
    ...
    public async Task<Result<MarkAsPlacedResponse>> Handle(MarkAsPlacedCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation(_messageService.GetMessage(MessageKeys.HRApplications.LogMarkAsPlaced), request.ApplicationId);

        try
        {
            if (!Guid.TryParse(_currentUserService.UserId, out var currentUserId))
                return Result<MarkAsPlacedResponse>.Failure(
            ...
            if (app.Status != InternshipApplicationStatus.Offered)
                return Result<MarkAsPlacedResponse>.Failure(
            ...
            {
                await _publisher.Publish(new ApplicationPlacedSelfApplyEvent(
                    studentUserId.Value,
                    app.ApplicationId,
                    app.Enterprise?.Name ?? string.Empty), cancellationToken);
            }

            return Result<MarkAsPlacedResponse>.Success(new MarkAsPlacedResponse
            {
                ApplicationId = app.ApplicationId,
                Status = app.Status,
                StatusLabel = app.Status.ToString(),
                UpdatedAt = app.UpdatedAt ?? DateTime.UtcNow
            }, _messageService.GetMessage(MessageKeys.HRApplications.LogMarkAsPlaced));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error marking application {ApplicationId} as placed", request.ApplicationId);
            return Result<MarkAsPlacedResponse>.Failure(
