        ...
        public async Task<Result<RestoreEnterpriseResponse>> Handle(RestoreEnterpriseCommand request, CancellationToken cancellationToken)
        {
            try {
                ...
                if (_currentUserService.Role != "SuperAdmin") {
                    bool canRestore = await _unitOfWork.Repository<Domain.Entities.EnterpriseUser>().ExistsAsync(x => x.UserId == CurrentUserHelper.GetValidGuidUserId(_currentUserService.UserId!) && x.EnterpriseId == request.EnterpriseId, cancellationToken);
                    if (!canRestore)
                    {
